...
| `nb0_ecgi` to `nb3_ecgi` | string | Optional. When exported with ``--include-neighbour-ecgi``, neighbour global identity in the same `mcc-mnc-tac-cid` form as ``cgi``, resolved by matching PCI+EARFCN to serving rows in this dataset (same row PLMN). Blank when unknown or ambiguous source data. |
...
Optional ``nb*_ecgi`` columns are documented when present; they are omitted from
the default export unless curation is run with ``--include-neighbour-ecgi``.
